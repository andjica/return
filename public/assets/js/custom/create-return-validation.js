...
//setup before functions
    ...
    $('#custom-return').on('submit', function(e){
        ...
        if(shablon == "user-r")
        {
            ...
            if(document.getElementById('return_photos').files.length == 0)
            {
                document.getElementById('er-photos').innerHTML="You must send photos of product for return";
                e.preventDefault();
            }
            else if(document.getElementById('return_photos').files.length >= 1)
            {   
                if (!document.getElementById('return_photos').files[0].name.match(/.(jpg|jpeg|png)$/i))
                    {
             ...
             //video validations
            if(document.getElementById('return_videos').files.length >= 1)
            {
               if (!document.getElementById('return_videos').files[0].name.match(/.(webm|mkv|flv|mp4)$/i))
                    {
            ...
            //onchange
            $("#return_photos").on('change' , function(){
    
            var photos = $("#return_photos").val();
